...
Comandos `curl` copy-paste pra cada endpoint exposto pelo chatlab. Todos os exemplos assumem:
...
(Encaminhamento multimodal pro provedor ainda não está implementado — por enquanto, anexos ficam armazenados ao lado da mensagem mas não são enviados ao LLM.)
