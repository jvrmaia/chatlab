...
3. **Read the template** by inspecting the most recent prior review for shape (`docs/reviews/2026-05-03-post-security-sprint.md` is canonical). Copy its skeleton:
...
- **Action register is the durable artifact.** Each row needs a target release (`v0.1.0`, `v0.2.0`, …) and a status (`Open`, `In progress`, `Closed YYYY-MM-DD — <evidence link>`, `Spec drafted`, `Skeleton`).
