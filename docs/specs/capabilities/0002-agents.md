...
- API keys are encrypted at rest with AES-256-GCM (see [`SECURITY.md`](https://github.com/jvrmaia/chatlab/blob/main/SECURITY.md#at-rest-encryption)); ciphertext is stored in the adapter. The memory adapter keeps plaintext in-process only.
...
- **Per-agent timeout / temperature in the API.** Defaults: 60 s, 0.7. Hardcoded for v0.1.0.
...
- **Multimodal input** to providers. Even though the workspace stores media, the runner does not forward images / audio to the provider in v0.1.0. Future capability.
- **Cost / token tracking.** Provider dashboards are the source of truth.
- **Full-row encryption** of conversation content (messages, feedback) — API keys only are encrypted at rest; message content is not.
