...
- The UI calls every endpoint with `Authorization: Bearer ui-dev-token` — the literal string is hardcoded in `src/ui/api.ts`. When `CHATLAB_REQUIRE_TOKEN` is set on the server, the bundled UI cannot call the API; either set the token to `ui-dev-token` or run the UI in an environment that injects the configured token (deferred to v0.2.0).

## Out of scope

- **Mobile-responsive layout.** Targeted at desktop laptops. Tablet / phone usability is best-effort.
- **i18n.** Bilingual en-US / pt-BR is implemented in v0.1.0 via `react-i18next`. Additional locales are future work.
- **Theming / dark mode.** Light and dark themes are implemented. Additional colour schemes are future work.
- **Search across chats.**
...
- **Vitest test ID(s):** none directly — UI is exercised by humans + by `npm run docs:capture` (Playwright screenshot pipeline). E2E regression deferred to v0.2.0 (item 12 of TRB review 2026-04-30).
