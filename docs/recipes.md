...
Copy-paste `curl` commands for every endpoint exposed by chatlab. All examples assume:
...
(Multimodal forwarding to the provider is not yet implemented — for now, attachments are stored alongside the message but not sent to the LLM.)
