...
- Does not dispatch webhooks (no built-in webhook surface — see [Roadmap](../ROADMAP.md))
...
v0.1.0 is the first release under the `chatlab` name. Tagged releases publish to npm under the `latest` dist-tag (stable) or `next` (pre-release).
