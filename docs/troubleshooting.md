...
`test/storage/duckdb.battery.test.ts` skips one media-related case by design — DuckDB's BLOB path is intentionally narrowed. The optional storage benchmark (`test/perf/storage-bench.test.ts`) is also skipped unless `CHATLAB_TEST_PERF=1`. Expect `113 passed | 2 skipped`.
